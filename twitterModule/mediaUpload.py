...
def APPENDMedia(filePath, mediaID):
    ...
    while True:
        ...
        if len(data) == 0:
            break
        req_body = {"command": "APPEND", "media_id": mediaID, "segment_index": index}
        req_files = {"media": data}
        response = requests.post(
            apiEndPoint, data=req_body, files=req_files, auth=oauth
        )
...
def FINALIZEMedia(mediaID):
    apiEndPoint = f"https://upload.twitter.com/1.1/media/upload.json"
    request_data = {"command": "FINALIZE", "media_id": mediaID}
